...
-- 8 Sep 2026 FIX: access-code hashing uses PostgreSQL built-in md5(); no digest() dependency.
...
begin
  ...
  raw:='SL2-'||upper(substr(replace(gen_random_uuid()::text,'-',''),1,6))||'-'||upper(substr(replace(gen_random_uuid()::text,'-',''),1,6))||'-'||upper(substr(replace(gen_random_uuid()::text,'-',''),1,6))||'-'||upper(substr(replace(gen_random_uuid()::text,'-',''),1,6));
  insert into public.canopy_team_access_codes(code_hash,code_hint,role,module_id,expires_at,created_by)
  values(md5(upper(trim(raw))),'••••-'||right(raw,6),p_role,p_module_id,exp,auth.uid())
...
begin
  ...
  foreach staff_role in array array['programme_manager','programme_operations'] loop
    raw:='SL2-'||upper(substr(replace(gen_random_uuid()::text,'-',''),1,6))||'-'||upper(substr(replace(gen_random_uuid()::text,'-',''),1,6))||'-'||upper(substr(replace(gen_random_uuid()::text,'-',''),1,6))||'-'||upper(substr(replace(gen_random_uuid()::text,'-',''),1,6));
    insert into public.canopy_team_access_codes(code_hash,code_hint,role,module_id,expires_at,created_by)
    values(md5(upper(trim(raw))),'••••-'||right(raw,6),staff_role,null,exp,auth.uid()) returning id into rid;
  ...
  foreach mod in array array['module-01','module-02','module-03','module-04','module-05'] loop
    ...
    raw:='SL2-'||upper(substr(replace(gen_random_uuid()::text,'-',''),1,6))||'-'||upper(substr(replace(gen_random_uuid()::text,'-',''),1,6))||'-'||upper(substr(replace(gen_random_uuid()::text,'-',''),1,6))||'-'||upper(substr(replace(gen_random_uuid()::text,'-',''),1,6));
    insert into public.canopy_team_access_codes(code_hash,code_hint,role,module_id,expires_at,created_by)
    values(md5(upper(trim(raw))),'••••-'||right(raw,6),'module_coordinator',mod,exp,auth.uid()) returning id into rid;
    ...
    for i in 1..3 loop
      raw:='SL2-'||upper(substr(replace(gen_random_uuid()::text,'-',''),1,6))||'-'||upper(substr(replace(gen_random_uuid()::text,'-',''),1,6))||'-'||upper(substr(replace(gen_random_uuid()::text,'-',''),1,6))||'-'||upper(substr(replace(gen_random_uuid()::text,'-',''),1,6));
      insert into public.canopy_team_access_codes(code_hash,code_hint,role,module_id,expires_at,created_by)
      values(md5(upper(trim(raw))),'••••-'||right(raw,6),'learning_fellow',mod,exp,auth.uid()) returning id into rid;
