...
export default function CanopyCanvas(){
  const initial={cause:causes[0],observance:'',audience:audiences[0],actionTarget:actions[0],tone:tones[0],format:formats[0],palette:'canopy',texture:'none',icons:['leaf'],message:''};
  ...
  const makeBlob=()=>new Promise(async resolve=>{
    ...
    ctx.fillStyle=palette.text;ctx.font=`700 ${Math.round(w*(form.tone==='Professional'?.050:.056))}px Arial`;
    ...
    ctx.fillStyle=palette.accent;ctx.font=`700 ${Math.round(w*.025)}px Arial`;wrap(ctx,form.actionTarget,pad,h*.80,w*.82,h*.04,3);
    ...
    form.icons.forEach((id,i)=>drawSimpleIcon(ctx,id,w*.78+i*w*.06,h*.13,w*.065,palette.accent));
    ...
    if(logo){
      ...
      ctx.globalAlpha=.55;ctx.fillStyle=palette.text;ctx.font=`700 ${Math.round(w*.018)}px Arial`;ctx.fillText('CANOPY · CLIMATE LEARNING BY WOMATE',w*.64,h*.96);ctx.globalAlpha=1;
    }
  ...
  return <section className="cc-page">
    ...
    <div className="cc-grid cc-grid-upgraded">
      <div className="cc-form">
        ...
        <label>Message
          <textarea rows="5" value={form.message} onChange={e=>update('message',e.target.value)} placeholder={suggestion}/>
          <span className="cc-message-helper"><Sparkles size={14}/> Tone suggestion: {suggestion}</span>
      ...
      <div className={`cc-preview cc-preview-upgraded ${form.format.toLowerCase().replace(/\s+/g,'-')} tone-${form.tone.toLowerCase().replace(/[^a-z]+/g,'-')}`}
