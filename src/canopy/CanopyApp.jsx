...
function LearnerShell({viewer,progress,children,onReload}){
 ...
 return <div className="canopyApp"><CourseNav open={open} setOpen={setOpen} viewer={viewer}/><div className="canopyMain"><header className="canopyAppTop"><button className="canopyMenu" onClick={()=>setOpen(true)} aria-label="Open navigation"><Menu/></button><div>{manager&&<span>WOMATE · CANOPY OPERATIONS</span>}<b>{name}</b></div><button className="canopyTopNotification" aria-label={unread?`${unread} unread notification${unread===1?'':'s'}`:'Notifications'} onClick={()=>go('/canopy/notifications')}><Bell size={18}/>{unread>0&&<span className="canopyNotificationBadge">{unread>99?'99+':unread}</span>}</button><button className="canopySignout" onClick={async()=>{await signOut();go('/canopy/login')}}><LogOut size={17}/><span>Sign out</span></button></header>{children}</div></div>
}
