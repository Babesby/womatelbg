...
function Dashboard({viewer,progress,submissions=[]}){
 ...
 return <main className="canopyDashboard cx-dashboard"><section className="canopyWelcome cx-welcome"><div><span className="canopyEyebrow">MY CANOPY</span><h1>Grow through the work.</h1><p>{enrolled?'Your She Leads course is active. Canopy will keep the next useful thing in view.':`Your account is ready. Canopy learning access opens from ${CANOPY_ACCESS_DATE}.`}</p></div><div className="canopyProgressRing" style={{'--p':`${pct*3.6}deg`}}><div><strong>{pct}%</strong><span>complete</span></div></div></section>{!enrolled?<section className="canopyLockedState"><Lock/><div><h2>Awaiting cohort enrolment</h2><p>Selected participants can be activated by WOMATE from {CANOPY_ACCESS_DATE}. Module 01 opens 21 September.</p></div></section>:<><CanopyThisWeek progress={progress} submissions={submissions} next={next}/><CanopyJourneyMap progress={progress} compact/><section className="canopyDashGrid cx-dashboard-grid"><article><small>COURSE</small><h3>She Leads Climate Mentorship</h3><Meta/><button onClick={()=>go('/canopy/course/she-leads')}>Open course <ArrowRight size={15}/></button></article><article><small>YOUR PROGRESS</small><h3>{completed} of {totalLessons} lessons complete</h3><div className="canopyLine"><span style={{width:`${pct}%`}}/></div><p>Short lessons, five knowledge checks and five applied assignments.</p></article><article><small>PORTFOLIO</small><h3>Build evidence as you learn.</h3><p>Your practical challenge links are organised into a professional She Leads portfolio.</p><button onClick={()=>go('/canopy/portfolio')}>Open portfolio <ArrowRight size={15}/></button></article></section><CanopySpotlight session={viewer.session}/></>}</main>
}
...
function Progress({progress,submissions=[]}){
 ...
 return <main className="canopyProgressPage"><div className="canopyPageHead"><span className="canopyEyebrow">YOUR LEARNING RECORD</span><h1>Progress</h1><p>Your progress is based on completed lessons and submitted assignments.</p></div><section className="canopyProgressHero"><div><strong>{pct}%</strong><span>lesson completion</span></div><div><strong>{completed}/{totalLessons}</strong><span>lessons</span></div><div><strong>{submittedModuleIds.size}/{modules.length}</strong><span>assignments submitted</span></div></section><CanopyJourneyMap progress={progress} compact/><section className="canopyProgressModules">{modules.map(m=>{const n=m.lessons.filter(l=>done.has(l.id)).length;const mp=Math.round(n/m.lessons.length*100);return <article key={m.id}><span>{m.number}</span><div><h3>{m.title}</h3><div className="canopyLine"><span style={{width:`${mp}%`}}/></div><small>{n}/{m.lessons.length} lessons · {submittedModuleIds.has(m.id)?'assignment submitted':'assignment pending'}</small></div><strong>{mp}%</strong></article>})}</section><CanopyReflectionTimeline/></main>
}
