...
const canopyDisplayWeek=w=>String(w).replace(/Weeks?\s*1(?:â€“|â€“|-)2/i,'Week 1 & 2').replace(/Weeks?\s*5(?:â€“|â€“|-)6/i,'Week 5 & 6').replace(/Weeks?\s*7(?:â€“|â€“|-)8/i,'Week 7 & 8');

function Brand({compact=false}){return <button className={'canopyBrand '+(compact?'compact':'')} onClick={()=>go('/canopy')} aria-label="WOMATE Canopy home"><img src="/assets/canopy/canopy-logo-primary.png" alt="Canopy â€” Climate Learning by WOMATE"/></button>}
...
function Landing(){return <div className="canopyPublic"><PublicTop/><main>
  ...
  <section className="canopyCourseIntro canopyCourseIntroEditorial" id="canopy-course"><div className="canopyCourseIntroLead"><span className="canopyEyebrow">SHE LEADS Â· COHORT 2</span><h2>A foundational climate leadership journey for young women who want to gain the knowledge, skills and courage to lead climate action.</h2></div><Meta/></section>
 ...
 </main><footer className="canopyPublicFooter"><div className="canopyFooterBrand"><img src="/assets/canopy/canopy-logo-primary.png" alt="Canopy â€” Climate Learning by WOMATE"/></div><div className="canopyFooterMeta"><span>Driving inclusive climate action with technology.</span><span>WOMATE learning environment</span></div></footer></div>}
function Auth({mode='login'}){
 ...
 const submit=async e=>{e.preventDefault();setBusy(true);feedback('');try{
   if(mode==='login'){
     ...
     if(form.team_code.trim()){
       ...
       feedback('Team access activated. Opening your WOMATE workspaceâ€¦','success');
     }else feedback('Signed in successfully. Opening Canopyâ€¦','success');
     ...
     if(data?.access_token){
       if(form.team_code.trim()){
         ...
         sessionStorage.removeItem('canopy_team_access_code_pending');
         feedback('Account created and team access activated. Opening your WOMATE workspaceâ€¦','success');
       ...
       feedback('An account may already exist for this email. Try signing in, or use â€œResend confirmationâ€ if the account is still unconfirmed.','warning')
     }else{
 ...
 return <div className="canopyAuth canopyAuthLuxe">
  ...
  <aside className="canopyAuthVisual">
   ...
   <button className="canopyAuthLogo" onClick={()=>go('/canopy')} aria-label="Canopy home"><img src="/assets/canopy/auth/canopy-logo-white.png" alt="Canopy â€” Climate Learning by WOMATE"/></button>
   <div className="canopyAuthVisualCopy">
    <span className="canopyAuthKicker">SHE LEADS Â· COHORT 2</span>
  ...
  <main className="canopyAuthPanelWrap">
   ...
   <section className="canopyAuthPanel">
    ...
    <div className="canopySocialAuth"><button type="button" className="canopyGoogleAuth" disabled={busy} onClick={async()=>{setBusy(true);feedback('');try{if(form.team_code.trim())sessionStorage.setItem('canopy_team_access_code_pending',form.team_code.trim());await signInWithGoogle()}catch(err){feedback(err.message||'Unable to continue with Google.','error');setBusy(false)}}}><span className="canopyGoogleMark" aria-hidden="true">G</span><span>Continue with Google</span></button><div className="canopyAuthDivider"><span>or continue with email</span></div></div><form onSubmit={submit}>
     ...
     <button className="canopyPrimary full canopyAuthSubmit" disabled={busy||!canopyConfigured}>{busy?'Please waitâ€¦':mode==='login'?'Sign in':'Create account'} <ArrowRight size={17}/></button>
...
function CourseNav({open,setOpen,viewer}){
 ...
 return <aside className={'canopySidebar '+(open?'open':'')}>
  ...
  <div className="canopySideBottom"><small>{manager?'WOMATE OPERATIONS':isCanopyOperationsStaff(viewer)?'WOMATE DELIVERY TEAM':tester?'CANOPY TEST MODE':CANOPY_BRAND.programme}</small><b>{manager?'She Leads Climate Mentorship Â· Cohort 2 Â· 2026':isCanopyOperationsStaff(viewer)?teamRoleLabel(viewer.staffAccess?.role):tester?'All modules Â· all assignments Â· unrestricted':CANOPY_BRAND.cohort}</b></div>
 </aside>
}
...
function Dashboard({viewer,progress}){
 ...
 const nextScheduled=!next&&completed<totalLessons?modules.find(m=>!canopyModuleIsOpen(m)):null;
 return <main className="canopyDashboard"><section className="canopyWelcome"><div><span className="canopyEyebrow">MY CANOPY</span><h1>Grow through the work.</h1><p>{enrolled?'Your She Leads course is active. Continue from where you stopped.':`Your account is ready. Canopy learning access opens from ${CANOPY_ACCESS_DATE}.`}</p></div><div className="canopyProgressRing" style={{'--p':`${pct*3.6}deg`}}><div><strong>{pct}%</strong><span>complete</span></div></div></section>{!enrolled?<section className="canopyLockedState"><Lock/><div><h2>Awaiting cohort enrolment</h2><p>Selected participants can be activated by WOMATE from {CANOPY_ACCESS_DATE}. Module 01 opens 21 September.</p></div></section>:<><section className="canopyContinue"><div><span>CONTINUE LEARNING</span><h2>{next?next.m.title:nextScheduled?`${nextScheduled.title} opens ${canopyModuleOpenLabel(nextScheduled)}`:'Course learning complete'}</h2><p>{next?next.l.title:nextScheduled?'You are caught up with the cohort.':'Review your progress and final submissions.'}</p></div>{next?<button className="canopyPrimary" onClick={()=>go(`/canopy/course/she-leads/${next.m.id}/${next.l.id}`)}>Continue <ArrowRight size={17}/></button>:<button className="canopySecondary" onClick={()=>go(nextScheduled?'/canopy/course/she-leads':'/canopy/progress')}>{nextScheduled?'View schedule':'View progress'} <ArrowRight size={17}/></button>}</section><section className="canopyDashGrid"><article><small>COURSE</small><h3>She Leads Climate Mentorship</h3><Meta/><button onClick={()=>go('/canopy/course/she-leads')}>Open course <ArrowRight size={15}/></button></article><article><small>YOUR PROGRESS</small><h3>{completed} of {totalLessons} lessons complete</h3><div className="canopyLine"><span style={{width:`${pct}%`}}/></div><p>Short lessons, five knowledge checks and five applied assignments.</p></article></section></>}</main>
}
...
function Progress({progress,submissions=[]}){
 ...
 return <main className="canopyProgressPage"><div className="canopyPageHead"><span className="canopyEyebrow">YOUR LEARNING RECORD</span><h1>Progress</h1><p>Your progress is based on completed lessons and submitted assignments.</p></div><section className="canopyProgressHero"><div><strong>{pct}%</strong><span>lesson completion</span></div><div><strong>{completed}/{totalLessons}</strong><span>lessons</span></div><div><strong>{submittedModuleIds.size}/{modules.length}</strong><span>assignments submitted</span></div></section><section className="canopyProgressModules">{modules.map(m=>{const n=m.lessons.filter(l=>done.has(l.id)).length;const mp=Math.round(n/m.lessons.length*100);return <article key={m.id}><span>{m.number}</span><div><h3>{m.title}</h3><div className="canopyLine"><span style={{width:`${mp}%`}}/></div><small>{n}/{m.lessons.length} lessons Â· {submittedModuleIds.has(m.id)?'assignment submitted':'assignment pending'}</small></div><strong>{mp}%</strong></article>})}</section></main>
}
...
function Profile({viewer}){const p=viewer.profile||{};return <main className="canopyProfile"><div className="canopyPageHead"><span className="canopyEyebrow">LEARNER PROFILE</span><h1>{p.full_name||viewer.user?.user_metadata?.full_name||'Your profile'}</h1><p>Your Canopy identity is connected to your WOMATE learning account.</p></div><section><div><small>Email</small><b>{viewer.user?.email}</b></div><div><small>Country</small><b>{p.country||viewer.user?.user_metadata?.country||'â€”'}</b></div><div><small>Role</small><b>{isCanopyTester(viewer)?'tester':viewer?.staffAccess?.role?teamRoleLabel(viewer.staffAccess.role):(p.role||'learner')}</b></div><div><small>Course access</small><b>{isCanopyTester(viewer)?'Unrestricted test access':(viewer.enrollments?.some(e=>e.status==='active')?'Active':'Awaiting enrolment')}</b></div></section></main>}
...
function ManagerOperations({snapshot,session,onReload,view='overview'}){
 ...
 if(!snapshot)return <main className="canopyManager"><div className="canopyPageHead"><h1>Canopy operations</h1><p>Loading operational dataâ€¦</p></div></main>;
 ...
 const moduleLabel=s=>{const schedule=CANOPY_ASSIGNMENT_SCHEDULE.find(x=>x.weekKey===s.week_key);const module=modules.find(m=>m.id===schedule?.moduleId);return module?`Module ${module.number} Â· ${module.title}`:(s.week_key||'Weekly assignment')};
 ...
 return <main className="canopyManager canopyOps">
  <div className="canopyPageHead"><span className="canopyEyebrow">WOMATE Â· CANOPY</span><h1>{sectionTitle}</h1><p>{view==='overview'?'Operational control for She Leads Climate Mentorship Â· Cohort 2 Â· 2026.':'Manage this workstream from one authorised WOMATE account.'}</p></div>
  ...
  {view==='access'&&<section className="canopyManagerTable canopyManagerAccessTable"><header><b>Participant</b><b>Country</b><b>Access</b><b>Action</b></header>{learners.map(p=>{const e=enrolFor(p.user_id),active=e?.status==='active';return <div key={p.user_id}><span>{p.full_name||'Unnamed learner'}</span><span>{p.country||'â€”'}</span><span><b className={'canopyAccessStatus '+(active?'isActive':'isWaiting')}>{active?'Active':'Waiting'}</b></span><span><button className={active?'canopySecondary':'canopyPrimary'} disabled={busy===p.user_id} onClick={()=>activate(p.user_id,active?'inactive':'active')}>{busy===p.user_id?'Updatingâ€¦':active?'Deactivate':'Activate access'}</button></span></div>})}</section>}
  {view==='reviews'&&<section className="canopyReviewWorkspace">
   ...
   <div className="canopyReviewList">{reviewRows.length?reviewRows.map(s=>{const d=draftFor(s);const status=reviewStatusOf(s);const manual=s.review_source==='manual';return <article className="canopyReviewCard" key={s.id}>
    <header><div><small>{nameFor(s.user_id)} Â· {moduleLabel(s)} Â· Attempt {s.attempt_no}</small><h3>{manual?'Manually reviewed':'Automated review'}</h3><p>Submitted {new Date(s.submitted_at).toLocaleString()}</p></div><span className={`canopyReviewStatus is-${status}`}>{status.replaceAll('_',' ')}</span></header>
    ...
    <div className="canopyReviewAuto"><div><span>Automated score</span><strong>{s.auto_score!=null?`${s.auto_score}/100`:'â€”'}</strong></div><div><span>Band</span><strong>{s.score_band||'â€”'}</strong></div><div className="wide"><span>Automated feedback</span><p>{s.feedback_hint||'No automated feedback yet.'}</p></div></div>
    <form className="canopyManualReview" onSubmit={e=>{e.preventDefault();reviewSubmission(s)}}>
     ...
     <div className="canopyManualReviewFoot"><div><span>Current final</span><b>{s.final_score!=null?`${s.final_score}/100 Â· ${s.score_band||''}`:'Not finalised'}</b>{s.review_source&&<small>{s.review_source==='manual'?'Manual WOMATE review':'Automated formative baseline'}</small>}</div><button className="canopyPrimary" disabled={busy==='review-'+s.id}>{busy==='review-'+s.id?'Savingâ€¦':manual?'Update review':'Save review'} <ArrowRight size={15}/></button></div>
    </form>
...
function ManagerCertificatePanel({learners,certificates,session,busy,setBusy,setMsg,onReload}){
 ...
 const nameFor=id=>learners.find(p=>p.user_id===id)?.full_name||'Learner';
 return <div className="canopyCertificateAdmin"><form className="canopyOpsForm" onSubmit={issue}><div className="canopySectionHead"><span>ISSUE COMPLETION RECORD</span><h2>Certificate</h2><p>Issue only after WOMATE has confirmed completion eligibility, including attendance and required assignments.</p></div><div className="canopyOpsFormGrid"><label>Learner<select required value={learnerId} onChange={e=>setLearnerId(e.target.value)}><option value="">Select learner</option>{learners.map(p=><option key={p.user_id} value={p.user_id}>{p.full_name||'Unnamed learner'}{p.country?' Â· '+p.country:''}</option>)}</select></label><label className="wide">Viewable Google Drive certificate link<input required inputMode="url" value={driveUrl} onChange={e=>setDriveUrl(e.target.value)} placeholder="https://drive.google.com/â€¦"/></label></div><button className="canopyPrimary" disabled={busy==='certificate'}>{busy==='certificate'?'Issuingâ€¦':'Issue certificate'} <Award size={16}/></button></form><section className="canopyOpsList">{certificates.length?certificates.map(c=><article key={c.id}><div><small>CERTIFICATE Â· {nameFor(c.user_id)}</small><h3>{c.title}</h3><p>Issued {new Date(c.issued_at).toLocaleString()}</p><a href={c.drive_url} target="_blank" rel="noreferrer">Open certificate <ArrowRight size={14}/></a></div><span className="canopyAccessStatus isActive">issued</span></article>):<p>No certificates issued yet.</p>}</section></div>
}
...
function ManagerActionForm({form,setForm,learners,busy,onSubmit,types}){
 ...
 return <form className="canopyOpsForm" onSubmit={onSubmit}><div className="canopySectionHead"><span>NEW ACTION</span><h2>{types.length===1?labels[types[0]]:'Send learner communication'}</h2></div><div className="canopyOpsFormGrid"><label>Learner<select required value={form.learner_id} onChange={e=>setForm({...form,learner_id:e.target.value})}><option value="">Select learner</option>{learners.map(p=><option key={p.user_id} value={p.user_id}>{p.full_name||'Unnamed learner'}{p.country?' Â· '+p.country:''}</option>)}</select></label>{types.length>1&&<label>Type<select value={form.action_type} onChange={e=>setForm({...form,action_type:e.target.value})}>{types.map(t=><option key={t} value={t}>{labels[t]}</option>)}</select></label>}<label className="wide">Subject<input required value={form.subject} onChange={e=>setForm({...form,subject:e.target.value})} placeholder={types[0]==='certificate'?'She Leads Climate Mentorship Â· Cohort 2 Â· 2026':'Short operational subject'}/></label><label className="wide">Message<textarea value={form.message} onChange={e=>setForm({...form,message:e.target.value})} rows="5" placeholder="Add the relevant note, feedback or decision."/></label></div><button className="canopyPrimary" disabled={busy==='action'}>{busy==='action'?'Savingâ€¦':types[0]==='certificate'?'Issue certificate':'Save and send'} <ArrowRight size={16}/></button></form>
}
function ManagerActionList({actions,nameFor,busy,onResolve}){return <section className="canopyOpsList">{actions.length?actions.map(a=><article key={a.id}><div><small>{a.action_type.toUpperCase()} Â· {nameFor(a.learner_id)}</small><h3>{a.subject}</h3><p>{a.message||'No additional note.'}</p>{a.certificate_code&&<code>{a.certificate_code}</code>}</div><div className="canopyOpsListActions"><span className={'canopyAccessStatus '+(a.status==='open'?'isWaiting':'isActive')}>{a.status}</span>{a.status==='open'&&<button className="canopySecondary" disabled={busy===a.id} onClick={()=>onResolve(a)}>{busy===a.id?'Updatingâ€¦':'Resolve'}</button>}</div></article>):<p>No records here yet.</p>}</section>}
...
export default function CanopyApp(){
 ...
 if(loading)return <div className="canopyLoading"><Leaf/><span>Opening Canopyâ€¦</span></div>;
