...
function Dashboard({viewer,progress,submissions=[]}){
 ...
 return <main className="canopyDashboard cx-dashboard"><section className="canopyWelcome cx-welcome"><div><span className="canopyEyebrow">MY CANOPY</span><h1>Grow through the work.</h1><p>{enrolled?'Your course is active.':`Learning access opens ${CANOPY_ACCESS_DATE}.`}</p></div><div className="canopyProgressRing" style={{'--p':`${pct*3.6}deg`}}><div><strong>{pct}%</strong><span>complete</span></div></div></section>{!enrolled?<section className="canopyLockedState"><Lock/><div><h2>Awaiting cohort enrolment</h2><p>Selected participants can be activated by WOMATE from {CANOPY_ACCESS_DATE}.</p></div></section>:<><CanopyThisWeek progress={progress} submissions={submissions} next={next}/><CanopyJourneyMap progress={progress} compact/><section className="canopyDashStrip"><div className="canopyDashProgress"><small>YOUR PROGRESS</small><strong>{completed} of {totalLessons} lessons complete</strong><div className="canopyLine"><span style={{width:`${pct}%`}}/></div></div><div className="canopyDashActions"><button className="canopyPrimary" onClick={()=>go(next?`/canopy/course/she-leads/${next.m.id}/${next.l.id}`:'/canopy/course/she-leads')}>{next?'Continue learning':'Open course'} <ArrowRight size={15}/></button><button className="canopyTextBtn" onClick={()=>go('/canopy/portfolio')}>Portfolio <ArrowRight size={14}/></button></div></section><CanopySpotlight session={viewer.session}/></>}</main>
}
