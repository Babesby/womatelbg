...
import {ArrowRight,Bot,ChevronDown,Send,Sparkles} from 'lucide-react';
  ...
  return <div className={`askCanopyWidget ${open?'isOpen':''}`}>
    {open&&<section className="askCanopyPanel" aria-label="Ask Canopy help assistant">
      <header className="askCanopyHead">
        <div className="askCanopyAvatar" aria-hidden="true"><Bot size={21}/><span><Sparkles size={10}/></span></div>
      ...
      <footer><Sparkles size={12}/><span>Verified Canopy answers. No external AI service.</span></footer>
    </section>}
    ...
    <button type="button" className="askCanopyLauncher" onClick={()=>setOpen(value=>!value)} aria-expanded={open} aria-label={open?'Close Ask Canopy':'Open Ask Canopy'}>
      <span className="askCanopyLauncherAvatar"><Bot size={22}/><i><Sparkles size={9}/></i></span>
