...
function ReviewAssignments({viewer,data,preview,onRefresh}){
 ...
 const statusCounts=useMemo(()=>{const counts={all:rows.length,needs_action:0,auto_reviewed:0,needs_manual_review:0,revision_required:0,completed:0};for(const row of rows){const status=row.assessment_status||'submitted';if(counts[status]!=null)counts[status]+=1;if(['auto_reviewed','needs_manual_review','revision_required'].includes(status))counts.needs_action+=1}return counts},[rows]);
 const visibleRows=useMemo(()=>{const q=query.trim().toLowerCase();return rows.filter(row=>{const status=row.assessment_status||'submitted';const matchesFilter=filter==='all'||(filter==='needs_action'?['auto_reviewed','needs_manual_review','revision_required'].includes(status):status===filter);if(!matchesFilter)return false;if(!q)return true;return [row.learner_name,row.module_label,row.week_key,row.score_band,row.paragraph_response].some(v=>String(v||'').toLowerCase().includes(q))})},[rows,filter,query]);
 const filters=[['needs_action','Needs action'],['auto_reviewed','Auto-reviewed'],['needs_manual_review','Manual review'],['revision_required','Revision required'],['completed','Completed'],['all','All']];
 ...
 function startReview(s){setOpen(s.id);setScore(String(s.score??''));setFeedback(s.final_feedback||'');setDecision(s.assessment_status==='revision_required'?'revision_required':'completed')}
 return <section className="cstaffPanel cstaffReviewPanel">
   <header className="cstaffReviewHead"><div><small>ASSIGNMENTS</small><h2>Review dashboard</h2><p>Filter the queue, open the evidence and add a human review where it helps.</p></div><span>{visibleRows.length} shown</span></header>
   <div className="cstaffReviewTools">
     <div className="cstaffReviewFilters" role="tablist" aria-label="Assignment review filters">{filters.map(([key,label])=><button type="button" role="tab" aria-selected={filter===key} className={filter===key?'active':''} key={key} onClick={()=>{setFilter(key);setOpen('')}}><span>{label}</span><b>{statusCounts[key]??0}</b></button>)}</div>
     <label className="cstaffReviewSearch"><span className="sr-only">Search reviews</span><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search learner or module"/></label>
   </div>
   <ActionNotice text={msg}/>
   {visibleRows.length?<div className="cstaffReviewRows">{visibleRows.map(s=>{const status=s.assessment_status||'submitted';const closed=status==='completed';return <article key={s.id} className={open===s.id?'is-open':''}>
     <div className="cstaffReviewIdentity"><small>{s.module_label||moduleLabel(s.week_key)}</small><h3>{s.learner_name||'Learner'}</h3><span>{s.submitted_at?new Date(s.submitted_at).toLocaleString():''}</span></div>
     <div className="cstaffReviewScore"><strong>{s.score!=null?s.score:'—'}</strong><span>{s.score!=null?'/100':'score'}</span>{s.score_band&&<small>{s.score_band}</small>}</div>
     <div className="cstaffReviewEvidence"><p>{s.paragraph_excerpt||s.paragraph_response||'Submission received.'}</p><div className="cstaffEvidence">{s.canvas_link&&<a href={s.canvas_link} target="_blank" rel="noreferrer">Practical work</a>}{s.linkedin_link&&<a href={s.linkedin_link} target="_blank" rel="noreferrer">LinkedIn evidence</a>}</div>{s.final_feedback&&closed&&<p className="cstaffReviewFeedback"><b>Final feedback:</b> {s.final_feedback}</p>}</div>
     <div className="cstaffReviewState"><Status>{status}</Status>{preview?<span className="cstaffReadOnly">Preview only</span>:closed?<span className="cstaffReviewClosed">Closed</span>:open===s.id?<button className="cstaffAction subtle" onClick={()=>setOpen('')}>Cancel</button>:<button className="cstaffAction" onClick={()=>startReview(s)}>{status==='auto_reviewed'?'Add review':status==='revision_required'?'Review revision':'Review'}</button>}</div>
     {open===s.id&&!closed&&<div className="cstaffReviewEditor"><StaffForm><label>Score<input type="number" min="0" max="100" value={score} onChange={e=>setScore(e.target.value)}/></label><label>Decision<select value={decision} onChange={e=>setDecision(e.target.value)}><option value="completed">Completed</option><option value="revision_required">Revision required</option><option value="needs_manual_review">Keep under review</option></select></label><label className="wide">Feedback<textarea rows="4" value={feedback} onChange={e=>setFeedback(e.target.value)} placeholder="Short, useful feedback to the learner"/></label><button className="cstaffAction" disabled={busy} onClick={()=>save(s.id)}>{busy?'Saving…':'Save review'}</button></StaffForm></div>}
   </article>})}</div>:<Empty>No submissions match this filter.</Empty>}
 </section>
}
...
export default function CanopyStaffWorkspace({viewer,path,onSignOut}){
  ...
  if(!role||!NAV[role])return <main className="cstaffError"><ShieldAlert/><h1>Team access unavailable</h1><p>Your Canopy team access is not active.</p></main>;
  ...
  return <div className="cstaffApp">
    {isPreview&&<div className="cstaffPreviewBanner"><strong>ADMIN PREVIEW MODE</strong><span>{ROLE_LABELS[role]}{moduleId?` · ${moduleLabel(moduleId)}`:''}</span><button onClick={()=>nav('/canopy/manage/role-preview')}>Change role</button><button onClick={()=>nav('/canopy/manage')}>Exit preview</button></div>}
    ...
    <div className="cstaffMain">
      ...
      <main className="cstaffContent"><div className="cstaffPageHead"><span>{ROLE_LABELS[role]}</span><h1>{(NAV[role].find(x=>x[0]===view)||[])[1]||'Workspace'}</h1><p>{isPreview?'Review this role’s workspace and available functions.':'Your Canopy workspace for She Leads Cohort 2.'}</p></div>{content}</main>
    </div>
  </div>;
}
