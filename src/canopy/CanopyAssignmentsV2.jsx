...
export default function CanopyAssignmentsV2({viewer}){
  ...
  return <section className="ca-page">
    <div className="ca-head">
      <span className="ca-kicker">ASSIGNMENTS</span>
      <h1>Work that follows the cohort.</h1>
      <p>Each week opens on Monday. Start the paragraph response, CanopyCanvas graphic and optional puzzle immediately. The speaker challenge unlocks after Thursday’s live session. Complete all three required parts by Sunday.</p>
    </div>
    ...
    <div className="ca-stack">
      {available.map(item=>{
        ...
        const mayResubmit=count<3 && new Date()<=new Date(item.resubmitUntil);
        ...
        return <article className="ca-card" key={item.weekKey}>
          ...
          <div className="ca-threefold">
            ...
            <div><b>02</b><h3>CanopyCanvas campaign</h3><p>{course?.assignment?.canvasBrief||'Create the campaign graphic in CanopyCanvas, download it, upload it to your own Google Drive, make it viewable and attach the link.'}</p><a href="/canopy/canvas">Open CanopyCanvas →</a></div>
