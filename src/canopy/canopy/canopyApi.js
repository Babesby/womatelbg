...
async function parse(r){const txt=await r.text();let data=null;try{data=txt?JSON.parse(txt):null}catch{data=txt}if(!r.ok)throw new Error(data?.msg||data?.message||data?.error_description||data?.error||'Request failed');return data}
...
export async function signUp({email,password,full_name,country}){
  if(!canopyConfigured)throw new Error('Canopy authentication is not configured yet.');
  const r=await fetch(`${URL}/auth/v1/signup`,{method:'POST',headers:headers(),body:JSON.stringify({email,password,data:{full_name,country}})});
  return parse(r);
}
...
export async function getManagerSnapshot(session){
  ...
  const [profiles,enrollments,progress,submissions]=await Promise.all([
  ...
  return {profiles,enrollments,progress,submissions};
}
