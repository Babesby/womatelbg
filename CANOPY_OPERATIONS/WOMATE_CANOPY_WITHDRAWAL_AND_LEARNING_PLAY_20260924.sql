-- WOMATE Canopy · admin-only learner withdrawal · 24 September 2026
-- Additive. Preserves learner records while blocking participant access.
-- Does not delete Auth users, submissions, progress, complaints or certificates.
...
begin
  if auth.uid() is null or not public.canopy_is_womate_admin(auth.uid()) then
    raise exception 'Only WOMATE Admin can view learner withdrawals.';
  end if;
  ...
  ) order by w.withdrawn_at desc),'[]'::jsonb) into result
...
-- Preserve the existing Programme Manager / WOMATE Admin access function,
-- but prevent non-restored withdrawn learners from being reactivated.
